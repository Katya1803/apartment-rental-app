...
class SiteSettingsService {
  async getAllSettings(locale: string = 'vi'): Promise<SiteSetting[]> {
    const response = await adminApi.get<ApiResponse<SiteSetting[]>>(
  ...
  async getSettingByKey(key: string, locale: string = 'vi'): Promise<SiteSetting> {
    const response = await adminApi.get<ApiResponse<SiteSetting>>(
  ...
  async updateSetting(key: string, data: SiteSettingUpdateRequest): Promise<SiteSetting> {
    const response = await adminApi.put<ApiResponse<SiteSetting>>(
      `${API_ENDPOINTS.ADMIN.SITE_SETTINGS}/${key}`,
      data
    )
    return response.data.data
